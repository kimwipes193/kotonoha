...
export const friendMessages=sqliteTable('friend_messages',{id:text('id').primaryKey(),friendship:text('friendship').notNull(),owner:text('owner').notNull(),target:text('target').notNull(),clientId:text('client_id').notNull(),photo:text('photo'),body:text('body').notNull(),created:integer('created').notNull(),readAt:integer('read_at')},t=>[uniqueIndex('dm_retry').on(t.owner,t.clientId),index('dm_thread').on(t.friendship,t.created,t.id),index('dm_sender').on(t.owner,t.created),index('dm_unread').on(t.target,t.friendship,t.readAt)]);
