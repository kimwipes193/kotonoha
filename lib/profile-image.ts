export function validProfilePhoto(value:unknown,maxDimension=256,maxLength=100000):value is string{
 if(typeof value!=='string'||value.length>maxLength||!/^data:image\/jpeg;base64,[A-Za-z0-9+/]+={0,2}$/.test(value))return false;
 ...
 while(pos<bytes.length-2){if(bytes[pos++]!==255)return false;const marker=bytes[pos++];if(marker===218)return dimensions;if(marker===225)return false;const length=(bytes[pos]<<8)|bytes[pos+1];if(length<2||pos+length>bytes.length)return false;if(marker===192||marker===194){const h=(bytes[pos+3]<<8)|bytes[pos+4],w=(bytes[pos+5]<<8)|bytes[pos+6];if(w<1||h<1||w>maxDimension||h>maxDimension)return false;dimensions=true;}pos+=length;}
 return false;
 }catch{return false;}
}
...
export function normalizeProfilePhoto(value:string,maxDimension=256,maxLength=100000):string{
 if(value.length>Math.max(500000,maxLength*3)||!/^data:image\/jpeg;base64,[A-Za-z0-9+/]+={0,2}$/.test(value))throw new Error('photo-format');
 ...
 if(!validProfilePhoto(cleaned,maxDimension,maxLength))throw new Error('photo-format');
 return cleaned;
}
