...
export async function messageAction(database:D1Database,owner:string,target:string,friendship:string,data:any):Promise<Response|null>{
 ...
 if(data.action==='friend-message'){
  ...
  await database.prepare('INSERT INTO friend_messages(id,friendship,owner,target,client_id,body,photo,created) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(owner,client_id) DO NOTHING').bind(id,friendship,owner,target,data.clientId,data.body.trim(),photo,Date.now()).run();
  const saved=await database.prepare("SELECT id,friendship,body,photo FROM friend_messages WHERE owner=? AND client_id=?").bind(owner,data.clientId).first<{id:string;friendship:string;body:string;photo:string|null}>();
