...
export async function friendState(database:D1Database,owner:string){
 ...
 CASE WHEN f.status='accepted' THEN (SELECT COUNT(*) FROM friend_messages WHERE friendship=f.id AND target=? AND read_at IS NULL) ELSE 0 END unread,CASE WHEN f.status='accepted' THEN p.nickname END nickname,CASE WHEN f.status='accepted' THEN p.icon END icon,CASE WHEN f.status='accepted' THEN p.details END details,CASE WHEN f.status='accepted' THEN p.birthday END birthday FROM friendships f JOIN profiles p ON p.owner=CASE WHEN f.a=? THEN f.b ELSE f.a END WHERE (f.a=? OR f.b=?) AND f.status IN ('pending','accepted') AND NOT EXISTS(SELECT 1 FROM blocks b WHERE (b.owner=f.a AND b.target=f.b) OR(b.owner=f.b AND b.target=f.a)) ORDER BY f.created DESC`).bind(owner,owner,owner,owner,owner,owner).all();
