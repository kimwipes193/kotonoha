...
export default function ProfilePager({children,labels,ids}:{children:ReactNode;labels:string[];ids:string[]}){
 ...
 useEffect(()=>{const el=track.current;if(!el)return;current.current=0;setIndex(0);el.scrollLeft=0;let width=el.clientWidth;
 ...
 const observer=new ResizeObserver(measure);observer.observe(el);Array.from(el.children).forEach(child=>observer.observe(child));measure();return()=>observer.disconnect();},[identity]);
