...
export function IncomingEnvelope({onOpen}:{onOpen:()=>void}){
 const [opening,setOpening]=useState(false);const timer=useRef<ReturnType<typeof setTimeout>|null>(null);
 useEffect(()=>()=>{if(timer.current)clearTimeout(timer.current);},[]);
 function open(){if(opening)return;setOpening(true);if(window.matchMedia('(prefers-reduced-motion: reduce)').matches){onOpen();return;}timer.current=setTimeout(onOpen,950);}
 return <Localized><div className={'incoming-mail '+(opening?'is-opening':'')}><span className="mail-eyebrow">A LITTLE LETTER FOR YOU</span><button type="button" className="envelope-open" aria-label="封筒を開けて日記を読む" disabled={opening} onClick={open}><EnvelopeArt/></button><p role="status">{opening?'封筒を開けています…':'とん、とタップで開封。'}</p><small>どこかの誰かが、今日を分けてくれました。</small></div></Localized>;
}
