...
export function PlacedSticker({sticker,layout,onChange,disabled=false}:{sticker:string;layout:StickerLayout;onChange?:(layout:StickerLayout)=>void;disabled?:boolean}){
 ...
 const [selected,setSelected]=useState(false),[aspect,setAspect]=useState(1);
 useEffect(()=>{const el=root.current?.parentElement;if(!el)return;const measure=()=>{const r=el.getBoundingClientRect();if(r.height)setAspect(r.width/r.height);};const observer=new ResizeObserver(measure);observer.observe(el);measure();return()=>observer.disconnect();},[]);
 ...
 const size=12*layout.scale,center=stickerCenter(layout,aspect);
 ...
 function move(e:PointerEvent<HTMLDivElement>){const start=drag.current;if(!start||start.pointer!==e.pointerId||!onChange||disabled)return;e.preventDefault();
  if(start.mode==='move'){const area=1-12*start.layout.scale*Math.SQRT2/100;onChange(moveSticker(start.layout,start.layout.x+(e.clientX-start.x)/start.width/area*100,start.layout.y+(e.clientY-start.y)/start.height/(1-12*start.layout.scale*Math.SQRT2*aspect/100)*100));}
  else onChange(transformSticker(start.layout,{x:start.x-start.cx,y:start.y-start.cy},{x:e.clientX-start.cx,y:e.clientY-start.cy},aspect));
 }
 function end(e:PointerEvent<HTMLDivElement>){if(drag.current?.pointer===e.pointerId)drag.current=null;}
 if(!onChange)return <div ref={root} className="placed-sticker" style={style} aria-hidden="true"><span className="sticker-art">{sticker}</span></div>;
