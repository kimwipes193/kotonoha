...
export default function LetterReader({entry,signedIn,sampleOriginal}:{entry:Letter;signedIn:boolean;sampleOriginal:string}){
 ...
 return <Localized><div ref={reading} tabIndex={-1} className={'read-letter '+entry.paper+' diary-font-'+(entry.font||'sans')}><span>{entry.mood}</span>{entry.drawing&&<DrawingCanvas strokes={readDrawing(entry.drawing)}/>}<div className="sticker-canvas read-canvas"><p data-i18n-skip lang={showTranslation?target:undefined}>{showTranslation?translations[target]:entry.id==='sample'?sampleOriginal:entry.body}</p>{readStickers(entry).map((p,i)=><PlacedSticker key={i} sticker={p.sticker} layout={p.layout}/>)}</div></div><div className="translation-controls"><label>翻訳先 <select aria-label="翻訳先" value={target} disabled={busy} onChange={e=>{setTarget(e.target.value as Locale);setShowTranslation(false);setError('');}}>{[['ja','日本語'],['en','English'],['fr','Français'],['zh-CN','简体中文'],['ko','한국어']].map(([value,label])=><option value={value} key={value} lang={value}>{label}</option>)}</select></label><button className="translate" disabled={busy||!entry.body.trim()} onClick={convert}><Languages size={16}/>{busy?'翻訳中…':showTranslation?'原文を見る':'この言語で読む'}</button></div>{entry.drawing&&<p className="translation-note">手書き部分は翻訳されません。</p>}<p className="translation-note">{entry.id==='sample'&&!signedIn?'見本はあらかじめ用意した翻訳です。':'翻訳すると本文をCloudflare AIで処理します。機械翻訳には誤りが含まれることがあります。'}</p>{busy&&<p role="status" className="translation-note">言葉のお着替え中。少しお待ちください。</p>}{error&&<p role="alert" className="dialog-error">{error}</p>}</Localized>;
}
