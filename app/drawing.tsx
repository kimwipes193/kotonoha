...
export function DrawingCanvas({strokes,onChange,disabled=false}:{strokes:Stroke[];onChange?:(s:Stroke[])=>void;disabled?:boolean}){
 ...
 function undoStroke(){if(!onChange||disabled||active.current||!strokes.length)return;setRedo([...redo,strokes[strokes.length-1]]);onChange(strokes.slice(0,-1));}
 function redoStroke(){if(!onChange||disabled||active.current||!redo.length)return;onChange([...strokes,redo[redo.length-1]]);setRedo(redo.slice(0,-1));}
