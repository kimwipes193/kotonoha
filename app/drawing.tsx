...
export function DrawingCanvas({strokes,onChange,disabled=false,children}:{strokes:Stroke[];onChange?:(s:Stroke[])=>void;disabled?:boolean;children?:ReactNode}){
 ...
 {onChange&&<div className="drawing-tools"><div className="drawing-actions"><button type="button" aria-pressed={!erase} aria-label="筆" title="筆" onClick={()=>setErase(false)}><Pencil size={22}/></button><button type="button" aria-pressed={erase} aria-label="消しゴム" title="消しゴム" onClick={()=>setErase(true)}><Eraser size={22}/></button><button type="button" className="drawing-history" aria-label="元に戻す" title="元に戻す" aria-keyshortcuts="Control+Z Meta+Z" disabled={disabled||(!strokes.length&&!undo.length)} onClick={undoStroke}><HistoryArrow/></button><button type="button" className="drawing-history" aria-label="やり直す" title="やり直す" aria-keyshortcuts="Control+Y Meta+Y Meta+Shift+Z" disabled={disabled||!redo.length} onClick={redoStroke}><HistoryArrow forward/></button><button type="button" aria-label="全消去" title="全消去" disabled={disabled||!strokes.length} onClick={()=>{if(active.current)return;setUndo([...undo,strokes]);setRedo([]);change([]);}}><Trash2 size={22}/></button></div><label>色 <input aria-label="ペンの色" className="hue-slider" type="range" min="0" max="359" value={hue} onChange={e=>chooseHue(Number(e.target.value))}/><span className="ink-preview" style={{background:ink}}/></label><div className="ink-swatches">{['#242132','#ffffff','#e65e66','#f4ad37','#51a884','#568bcc','#9368b7'].map(color=><button key={color} type="button" aria-label={color} aria-pressed={ink===color&&!erase} style={{background:color}} onClick={()=>chooseSwatch(color)}/>)}</div><label>明度 <input aria-label="ペンの明度" className="brightness-slider" style={{background:`linear-gradient(to right,#000,${hsvToHex(hue,saturation,100)})`}} type="range" min="0" max="100" value={brightness} onChange={e=>chooseBrightness(Number(e.target.value))}/><output>{Math.round(brightness)}%</output></label><label>太さ <input aria-label="ペンと消しゴムの太さ" type="range" min="1" max="40" value={width} onChange={e=>setWidth(Number(e.target.value))}/><output>{width}</output></label>{full&&<p role="status">画数の上限です。元に戻すと描き直せます。</p>}</div>}
