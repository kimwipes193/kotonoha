...
assert.equal(checkDiary('あ'),null);
assert.equal(checkDiary('あ'.repeat(200)),null);
assert.ok(checkDiary('あ'.repeat(201)));
assert.equal(checkDiary('',true),null);
assert.ok(checkDiary('あ'.repeat(201),true));
assert.equal(checkDiary('あ'.repeat(1000),false,1000),null);
assert.equal(checkDiary('今日は公園を散歩して、きれいな空を見上げました。'),null);
for(const text of ['','   ','連絡は someone@example.com までお願いします。','電話番号は０９０１２３４５６７８です。よろしくお願いします。','詳しいことは https://example.com を読んでください。','今日はつらかった。死ねと言ってしまった。']) assert.ok(checkDiary(text));
